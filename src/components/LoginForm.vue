  ...
  methods:{
    ...
    processLogin(){
      ...
      if(this.$data.username==''){
        ...
        axios.post('http://app.abhiyantrik.com/api/v1/checkwarduser',{'username':this.$data.username}).then(response=>{
          var formData = {'username': this.$data.username, 'password': this.$data.password}
          axios.post('http://app.abhiyantrik.com/api/v1/token',formData)
          .then(response => {
